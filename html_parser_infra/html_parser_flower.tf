# network interface
resource "aws_network_interface" "html-parser-flower-interface" {
  subnet_id       = var.PRIVATE_SUBNET_ID
  private_ips     = [jsondecode(data.aws_secretsmanager_secret_version.secret_version.secret_string)["PRIVATE_IP_FLOWER"]]
  security_groups = [aws_security_group.html_parser_sg.id]

  tags = {
    Name = "network_interface_for_html-parser-flower"
  }
}

# Instance for html-parser-flower
resource "aws_instance" "html-parser-flower" {
  ami           = var.EC2_AMI
  instance_type = "t3.small"
  key_name      = "html-parser-terraform"

  network_interface {
    network_interface_id = aws_network_interface.html-parser-flower-interface.id
    device_index         = 0
  }
  tags = {
    Name    = "AWS-GBD-htmlparser_flower_${terraform.workspace}"
    Project = "html_parser"
    Stage   = terraform.workspace
  }
  volume_tags = {
    Name = "htmlparser_flower_volume_${terraform.workspace}"
  }
  # subnet_id              = var.PRIVATE_SUBNET_ID
  user_data = base64encode(templatefile("userdata/flower_user_data.tftpl", {
